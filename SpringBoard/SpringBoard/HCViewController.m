...
@property (nonatomic, strong) NSMutableArray *iconModelsArray;//安装的app(icon+floder)
...
{
    //http://storemarket.zhengtoon.com/app/index.html?phoneflag=1
    AppDelegate *del = (AppDelegate *)[UIApplication sharedApplication].delegate;
    HCBankListViewController *menuListViewController = [[HCBankListViewController alloc] init];
    menuListViewController.mainMenuList = self.favoriteMainMenu.itemList;
    menuListViewController.allMenuModels = self.iconModelsArray;
...
- (void)displayMenu {
    ...
    NSData *userDefaultsLoveMenuArray = [userDefaultsLoveMenu objectForKey:kUserDefaultLoveMenuKey];
    if (userDefaultsLoveMenuArray) {
        NSArray *modelsArray = [NSKeyedUnarchiver unarchiveObjectWithData:userDefaultsLoveMenuArray];
    ...
    [self.speechKeysArray addObjectsFromArray:@[@"水费",@"燃气费",@"公积金",@"社保",@"电费",@"设置"]];
    //根据数据显示菜单
    if (_springBoard.superview) {
...
{
    if ([favoroteModel.type isEqualToString:kViewcontroller]) {
        if (favoroteModel.display) {
            [_iconModelsArray addObject:favoroteModel];
        }
    }
    else if ([favoroteModel.type isEqualToString:kMenuList]) {
